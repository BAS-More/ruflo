...
// Card in grid view
function Card({ card, setActiveCard }: { card: Card; setActiveCard: (card: Card | null) => void }) {
  return (
    <motion.div
    ...
    >
      ...
      <motion.div layoutId={`card-content-${card.title}`} className="card-content">
        <motion.h2 layoutId={`card-heading-${card.title}`}>{card.title}</motion.h2>
        <motion.p layoutId={`card-description-${card.title}`}>{card.description}</motion.p>
      </motion.div>
  ...
  return (
    ...
    >
      <motion.img
        ...
        style={{ borderRadius: 0 }}
      />
      <motion.button className="close-button" onClick={() => setActiveCard(null)}>
        ✕
      </motion.button>
      <motion.div layoutId={`card-content-${activeCard.title}`} className="card-content">
        <motion.h2 layoutId={`card-heading-${activeCard.title}`}>{activeCard.title}</motion.h2>
...
export default function AppStoreCards() {
  ...
  return (
    <div className="cards-wrapper">
      ...
      {/* Expanded card */}
      <AnimatePresence>
        {activeCard && <ActiveCard activeCard={activeCard} setActiveCard={setActiveCard} />}
      </AnimatePresence>
    </div>
  );
}
